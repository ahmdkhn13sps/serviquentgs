...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const subject = isCareers
      ? 'New Job Application — ' + (b.service || 'Open Role') + ' — ' + (b.first_name || '') + ' ' + (b.last_name || '')
      : isQuote
        ? 'New Quote Request — ' + (b.company || b.first_name || 'Unknown')
        : 'New BPO Enquiry — '   + (b.company || b.first_name || 'Unknown');

    const html = isCareers ? `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
  <div style="background:linear-gradient(135deg,#1a6fc4,#0ea5c8);padding:28px 32px;border-radius:10px 10px 0 0">
    <h2 style="color:#fff;margin:0;font-size:20px">&#128101; New Job Application</h2>
    <p style="color:rgba(255,255,255,.8);margin:4px 0 0;font-size:13px">Received via serviquentgs.com/careers</p>
  </div>
  <div style="background:#fff;padding:28px 32px;border:1px solid #e5e7eb;border-top:none">
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      <tr style="border-bottom:1px solid #f3f4f6"><td style="padding:10px 0;width:30%;color:#6b7280;font-weight:600">Name</td><td style="padding:10px 0;color:#111;font-weight:500">${b.first_name || ''} ${b.last_name || ''}</td></tr>
      ...
      <tr style="border-bottom:1px solid #f3f4f6"><td style="padding:10px 0;color:#6b7280;font-weight:600">Role Applied For</td><td style="padding:10px 0"><span style="background:#eff6ff;color:#1a6fc4;padding:3px 10px;border-radius:20px;font-weight:600;font-size:12px">${b.service || 'Not specified'}</span></td></tr>
      <tr style="border-bottom:1px solid #f3f4f6"><td style="padding:10px 0;color:#6b7280;font-weight:600">Current City</td><td style="padding:10px 0;color:#111">${b.country || 'Not provided'}</td></tr>
      <tr style="border-bottom:1px solid #f3f4f6"><td style="padding:10px 0;color:#6b7280;font-weight:600">Experience</td><td style="padding:10px 0;color:#111">${b.team_size || 'Not specified'}</td></tr>
      <tr><td style="padding:10px 0;color:#6b7280;font-weight:600;vertical-align:top">About</td><td style="padding:10px 0;color:#374151;line-height:1.7">${(b.message || '').replace(/\n/g, '<br/>')}</td></tr>
    </table>
  </div>
  <div style="background:#f9fafb;padding:14px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px">
    <p style="margin:0;font-size:12px;color:#9ca3af">Submitted from <strong>serviquentgs.com/careers</strong></p>
  </div>
</div>` : `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
  <div style="background:linear-gradient(135deg,#0ea5c8,#1a6fc4);padding:28px 32px;border-radius:10px 10px 0 0">
    <h2 style="color:#fff;margin:0;font-size:20px">${isQuote ? '&#128179; New Quote Request' : '&#128231; New BPO Enquiry'}</h2>
  ...
  <div style="background:#fff;padding:28px 32px;border:1px solid #e5e7eb;border-top:none">
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      ...
      <tr><td style="padding:10px 0;color:#6b7280;font-weight:600;vertical-align:top">Message</td><td style="padding:10px 0;color:#374151;line-height:1.7">${(b.message || '').replace(/\n/g, '<br/>')}</td></tr>
    </table>
  </div>
  <div style="background:#f9fafb;padding:14px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px">
    ...
    await resend.emails.send({
      from: 'Serviquent Global Solutions <onboarding@resend.dev>',
